...
两边的分工正好对得上——**本服务不持有模型，讲稿由调用方写**，在 dsh 里调用方就是 dsh 的模型。

有两条路：

| | 这一页（MCP 桥接） | [插件](plugin/README.md) |
|---|---|---|
| 要写代码吗 | 不用，一段配置 | 已经写好了，装一个包 |
| 投文档 | base64 塞进工具调用，几 MB 的文件进不去 | **按本机路径**，字节不进对话 |
| 后端 | 自己先起 | 插件自己拉起来、自己收掉 |
| 工具名 | `mcp__doc2video__*`（8 个） | `doc2video_*`（5 个） |

先看这一页确认能连上，再换插件。下面是 MCP 这条路。
