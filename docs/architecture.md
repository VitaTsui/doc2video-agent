...
### Kokoro（2026-08-20 评估：值得，但不进运行时）

「配音太机械」不是玄学，是可以量的。同一段中文讲稿，同样长度的音频：

| 指标 | macOS `say` | Kokoro-82M |
| --- | --- | --- |
| 停顿离散度 | 0.13 | **0.66** |
| 最长连讲 | 2.0s | **4.2s** |
| 能量起伏 | 0.48 | 0.55 |
| 合成速度（热） | ~13× 实时 | 9.55× 实时 |
| 代价 | 系统自带 | torch + 82M 模型 |

关键那一行是**停顿离散度**。`say` 每次停顿几乎一样长——那就是「查标点表」的
签名；Kokoro 的停顿随上下文变化五倍，并且会连着讲 4 秒再换气，而不是每个标点
都停。这正是听感上「在讲」和「在念」的分界。速度不成问题：530 秒的片子约 55
秒配完。

决定：**接进 provider 链并排在最前，但不进运行时**。它带 torch，而桌面版首次
下载本来就是安装过程里最慢的一步——照 Docling 那条同样的理由。现在的行为是：
装了就自动用上，没装就跟以前完全一样落回 `say` / Piper。

按需安装用 `doc2video voice-upgrade`：往当前运行时里装 kokoro，装完下次生成
自动用上。单独一条命令而不是渲染时自动拉——几百兆的下载卡在做视频中间，跟
卡死没有区别（跟 Piper 语音模型是同一条规矩）。

### `say -r` 对中文几乎无效（同日测得）

顺带量出来的一件事，解释了「语速太快」为什么一直调不动：
...
降 25% 只慢了 9%——`-r` 是按英文「词/分钟」算的，而中文一个字不是一个词。
Kokoro 的 `speed` 是真正的时间伸缩：1.0 → 0.8 对应 316 → 257 字/分。

因此语速的默认值改由引擎自己声明（`TTSProvider.natural_rate`）：`say` 是
1.0，Kokoro 是 0.85（它自带的 316 字/分对技术文稿太快）。用户说「慢一点」是
乘在这个基准上，而不是替换它。

### 语音包（2026-08-20 试听定案）

同一段讲稿，17 个本地音色加 4 个在线音色一起听下来，选的是
**`zh-CN-YunyangNeural`（Edge，语速 −8%）**。微软给它的标签是
Professional / Reliable，是这批中文音色里唯一那个性格。

排除掉的和原因：

| | 结果 |
| --- | --- |
| macOS `say` | 稳、没口音，但每次停顿一样长；`-r` 对中文几乎无效 |
| Kokoro | 停顿有变化，但**有口音**——讲解场景反而不如 `say` |
| Piper huayan | Windows / Linux 的兜底，只此一个 |
| Azure style 标签 | 免费端点不认，把 `newscast` 当文字念了（11.3s → 17.3s） |

**`AUTO_ORDER` 仍然全本地**（say → kokoro → piper → silent）。Edge 要联网，
而「装完就能用、不起外部服务」是这个产品的前提——一个上不了火车就做不成的
视频是另一个产品。所以它必须被点名要，不会自动用上。

点名的方式有两条，都不用改环境变量：说「用播音腔讲」，或直接给音色名。
音色名自己决定用哪个引擎（`zh-CN-*` 归 Edge、`zm_*` 归 Kokoro、其余归系统），
因为桌面端没有地方让人设环境变量。

**断网不会毁掉一次渲染**：第一次失败就换成本机最好的声音，把剩下的讲完，
并记一条降级——一条前半段一个声音、后半段另一个声音的片子，做的人有权知道。
